...
mb_address = 33 # Modbus address of sensor

sensy_boi = minimalmodbus.Instrument('COM4',mb_address)	# Make an "instrument" object called sensy_boi (port name, slave address (in decimal))
...
while True:
    # data =sensy_boi.read_registers(2, 2, 3) 
    temp =round(sensy_boi.read_float(2),2)  # Read temperature from register 2 and round to 2 decimal places
    humid =sensy_boi.read_float(6)
    print("")
    ...
    print(f"Relative Humidity = {humid}%")
    print("-------------------------------------")
    print("")
